...
**선수 지식**: [핵심 구성요소]({{< relref "/docs/kafka/concepts/core-components" >}})의 Broker, Producer, Consumer 개념, 기본적인 보안 개념(암호화, 인증, 인가)
...
Kafka 보안을 <strong>오피스 빌딩 보안</strong>에 비유하면 이해하기 쉽습니다:

| 빌딩 보안 비유 | Kafka 보안 | 역할 |
|---------------|-----------|------|
| 지문 출입문 (암호화된 통로) | SSL/TLS | 도청 방지, 통신 암호화 |
| 사원증 리더기 (신원 확인) | SASL | "당신이 누구인지" 인증 |
| 층별 접근 권한 (출입 제한) | ACL | "어디까지 갈 수 있는지" 인가 |
| Keystore | 내 사원증 | 내 신원 증명 |
| Truststore | 신뢰할 수 있는 발급기관 목록 | 상대방 검증 기준 |

**3단계 보안 흐름**: 암호화된 문으로 들어가고(SSL) → 사원증을 찍고(SASL) → 허가된 층만 접근(ACL)
...
- [모니터링]({{< relref "/docs/kafka/concepts/monitoring" >}}) - 보안 이벤트 모니터링
- [생태계]({{< relref "/docs/kafka/concepts/ecosystem" >}}) - Schema Registry 보안 설정
