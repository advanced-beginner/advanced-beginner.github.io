...
acks는 Producer가 메시지 전송 성공을 어떻게 확인할지 결정하는 설정입니다. 이 설정에 따라 메시지 안전성과 전송 속도 사이의 균형이 달라집니다.
...
**acks=all**은 ISR에 있는 모든 복제본에 메시지가 복제된 후에야 ACK를 반환합니다. 가장 느리지만 가장 안전한 설정으로, 결제나 주문과 같이 데이터 유실이 치명적인 경우에 사용합니다.
...
Message Key는 메시지를 특정 Partition으로 라우팅하는 데 사용됩니다. Key가 있으면 같은 Key를 가진 메시지는 항상 같은 Partition에 저장되어 순서가 보장됩니다.
...
Retention은 메시지를 얼마나 오래 보관할지 결정합니다. Kafka는 세 가지 보관 정책을 제공합니다.

**시간 기반 삭제**는 지정된 시간이 지난 메시지를 삭제합니다. 기본값은 7일(604800000ms)입니다. 이벤트 로그는 7일, 감사 로그는 1년, 세션 데이터는 24시간과 같이 데이터 특성에 맞게 설정합니다.
...
**용량 기반 삭제**는 Partition의 총 크기가 지정된 용량을 초과하면 오래된 세그먼트부터 삭제합니다. 디스크 용량 관리가 필요한 경우에 사용합니다.

```yaml
retention.bytes: 107374182400  # 100GB
```

**Log Compaction**은 Key별 마지막 값만 유지하는 정책입니다. 시간이나 용량이 아니라 Key 중복 여부를 기준으로 정리합니다. 사용자 프로필이나 설정 상태처럼 최신 값만 필요한 경우에 적합합니다.
...
**고신뢰성 프로덕션 환경**에서는 데이터 안전성을 최우선으로 설정합니다.
...
**고성능 로깅 환경**에서는 처리량을 최우선으로 설정합니다.
