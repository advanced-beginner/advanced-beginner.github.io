...
**선수 지식**: [Consumer Group & Offset]({{< relref "/docs/kafka/concepts/consumer-group" >}})의 Consumer 동작 원리, Spring Kafka 기초
...
Consumer 에러 처리를 <strong>우편물 배달</strong>에 비유하면 이해하기 쉽습니다:

| 우편물 배달 비유 | Kafka 에러 처리 | 설명 |
|-----------------|----------------|------|
| 주소 오류 (읽을 수 없음) | 역직렬화 에러 | 메시지 자체가 잘못됨, 재시도 무의미 |
| 부재중 (나중에 재배달) | 일시적 에러 | 잠시 후 재시도하면 성공 가능 |
| 수취 거부 (반송) | 영구적 에러 | 재시도해도 실패, DLT로 보관 |
| 반송 우편함 | Dead Letter Topic | 처리 불가 우편물 보관소 |
| 재배달 시도 (1회, 2회...) | 재시도 정책 | 지수 백오프로 간격 늘려가며 재시도 |

핵심은 <strong>우편물 유형에 따라 다르게 처리</strong>하는 것입니다. 주소 오류는 재배달해도 소용없고, 부재중은 재배달하면 됩니다.
...
- [모니터링 기초]({{< relref "/docs/kafka/concepts/monitoring" >}}) - Kafka 모니터링 및 메트릭
