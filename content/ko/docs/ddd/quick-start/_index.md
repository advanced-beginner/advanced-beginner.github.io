...
title: Quick Start
description: "DDD 핵심 개념과 실습을 단계별로 학습합니다."
weight: 1
lastmod: "2026-01-13"
author: "@kimbenji"
...
- Bounded Context로 시스템을 분리하고, Aggregate로 트랜잭션 경계를 정의합니다
- 복잡한 비즈니스 규칙이 있는 시스템에서 효과적이며, 단순 CRUD에는 오버엔지니어링입니다
{{< /callout >}}
...
DDD를 처음 적용할 때 흔히 겪는 문제들과 해결 방법입니다.

**문제 1: "Aggregate 경계를 어디로 잡아야 할지 모르겠어요"**

```
증상: 모든 것을 하나의 Aggregate에 넣거나, 반대로 너무 작게 나눔
원인: 불변식(invariant) 개념을 명확히 이해하지 못함
```

**해결책:** "이 데이터들이 항상 함께 일관성을 유지해야 하는가?"를 질문하세요. 예를 들어, Order와 OrderLine은 함께 변경되어야 하므로 같은 Aggregate입니다. 하지만 Customer는 Order와 독립적으로 변경 가능하므로 별도 Aggregate입니다.

---

**문제 2: "Service에 비즈니스 로직이 자꾸 들어가요"**

```
증상: Entity는 getter/setter만, Service가 점점 비대해짐
원인: "Anemic Domain Model" 안티패턴
```

**해결책:** "이 로직이 Entity의 상태를 변경하는가?"를 질문하세요. 그렇다면 해당 Entity 안에 메서드로 넣으세요.
...
**문제 3: "Bounded Context를 어떻게 나눠야 할지 모르겠어요"**

```
증상: 하나의 거대한 모놀리식 도메인 모델
원인: Context 분리 기준 불명확
```

**해결책:** 다음 신호를 찾으세요:
- 같은 용어가 다른 의미로 사용됨 (예: "Product"가 판매팀과 물류팀에서 다른 속성을 가짐)
- 다른 팀이 담당하는 영역
- 다른 생명주기를 가지는 데이터

---

**문제 4: "JPA Entity와 Domain Entity를 분리해야 하나요?"**

```
증상: @Entity 어노테이션이 도메인 모델을 오염
원인: 인프라 관심사와 도메인의 경계 혼란
```

**해결책:** 프로젝트 규모에 따라 선택하세요:
- **소규모/단기:** 분리하지 않아도 됨 (실용적 접근)
- **중규모 이상:** Domain Model과 JPA Entity 분리 권장, Mapper로 변환
