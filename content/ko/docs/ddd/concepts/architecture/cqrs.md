...
CQRS 핵심: <strong>Command</strong>(상태 변경, 도메인 모델 사용) ↔ <strong>Query</strong>(조회, 최적화된 읽기 모델 사용) 분리로 각각의 요구사항에 맞게 최적화
{{< /callout >}}

{{< callout type="tip" title="비유: 도서관" >}}
CQRS를 <strong>도서관 운영</strong>에 비유할 수 있습니다:

- **기존 방식(단일 모델)**: 도서 관리자가 책 대출/반납도 처리하고, 도서 검색 요청도 처리합니다. 대출이 밀리면 검색도 느려지고, 검색이 많으면 대출도 느려집니다.
- **CQRS 방식**: 대출 담당자(Command)와 검색 사서(Query)를 분리합니다. 대출 담당자는 책의 상태를 변경하고, 검색 사서는 최적화된 색인(인덱스)을 활용하여 빠르게 응답합니다.

**핵심**: 대출(쓰기)과 검색(읽기)의 요구사항이 다르므로 각각에 최적화된 방식으로 처리합니다. 대출은 정합성이 중요하고, 검색은 속도가 중요합니다.
{{< /callout >}}
...
*전통적인 CRUD 방식에서 UI, Service, Model, DB가 하나의 경로로 모든 작업을 처리하는 구조를 보여줍니다.*
...
*CQRS 패턴에서 Command Side(쓰기)와 Query Side(읽기)가 분리되어 각각의 모델과 DB를 사용하는 구조입니다.*
...
*Level 1 CQRS로, 하나의 DB를 사용하면서 코드 레벨에서 명령과 조회를 분리하는 구조입니다.*
...
*Level 2 CQRS로, 쓰기는 정규화된 테이블에, 읽기는 비정규화된 테이블에서 수행하는 구조입니다.*
...
*Level 3 CQRS로, Write DB와 Read DB를 완전히 분리하고 이벤트 버스(Kafka)로 동기화하는 구조입니다.*
...
*CQRS와 Event Sourcing을 결합한 구조로, Command가 이벤트를 생성하고 이벤트로 Read Model을 구축합니다.*
...
| 시스템 유형 | 적합도 | 이유 |
|------------|-------|------|
| **대시보드/리포팅** | 매우 적합 | 복잡한 집계 쿼리, 다양한 조회 형태 |
| **이커머스 플랫폼** | 적합 | 상품 조회 최적화, 주문 이력 분석 |
| **검색 기능** | 적합 | Elasticsearch 등 별도 검색 엔진 활용 |
| **이벤트 소싱** | 매우 적합 | CQRS는 이벤트 소싱과 자연스러운 조합 |
| **마이크로서비스** | 적합 | 서비스별 독립적 확장 |
| **실시간 협업 도구** | 적합 | 이벤트 기반 동기화 |
| **은행 거래** | 부적합 | 즉시 일관성 필수 |
| **재고 시스템** | 상황에 따라 | 실시간 재고 확인 필요시 부적합 |
| **단순 관리 시스템** | 부적합 | 복잡도 대비 이점 적음 |
