...
Jeffrey Palermo가 2008년에 제안한 아키텍처입니다. **도메인 모델을 가장 중심**에 두고, 양파처럼 겹겹이 감싸는 구조입니다.

어니언 아키텍처는 전통적인 계층형 아키텍처의 한계에서 출발합니다. 계층형 아키텍처에서는 상위 계층이 하위 계층에 의존하므로, 데이터베이스 계층의 변경이 서비스 계층에 연쇄적인 영향을 미칩니다. 반면 어니언 아키텍처에서는 **도메인 모델이 어떤 것에도 의존하지 않고**, 인프라스트럭처가 도메인에 의존하는 방향으로 설계합니다. 이를 통해 도메인 로직은 데이터베이스, 프레임워크, 외부 서비스의 변화와 무관하게 순수한 비즈니스 규칙만을 표현할 수 있습니다. DDD(Domain-Driven Design)와 특히 잘 어울리는 이유도 여기에 있습니다.
...
| 양파 층 | 소프트웨어 층 | 특징 |
|--------|-------------|------|
| 바깥 껍질 | Infrastructure | 교체 가능, 기술적 세부사항 |
| 바깥쪽 | Application Services | 흐름 조율 |
| 안쪽 | Domain Services | 도메인 로직 조합 |
| 💎 핵심 | Domain Model | 절대 바뀌지 않는 비즈니스 규칙 |

**핵심 아이디어:** 도메인 모델은 가장 안쪽에서 **아무것에도 의존하지 않습니다**.
...
**여러 도메인 객체를 조합하는 로직**이 있는 곳입니다. 한 Entity에 넣기 어려운 로직을 담습니다.
...
**Use Case 흐름을 조율**합니다. 트랜잭션 관리, 외부 시스템 호출 등을 담당합니다.
...
**기술적 세부사항**이 있는 곳입니다. UI, 데이터베이스, 외부 API 연동 등.
...
> **핵심:** 아키텍처의 복잡성은 **해결하려는 문제의 복잡성에 비례**해야 합니다. 단순한 문제에 복잡한 해결책을 적용하면, 그 복잡성 자체가 새로운 문제가 됩니다.
