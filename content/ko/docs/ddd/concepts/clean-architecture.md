...
Uncle Bob(Robert C. Martin)이 2012년에 제안한 아키텍처입니다. **의존성 규칙**을 엄격하게 지키는 것이 핵심입니다.

클린 아키텍처가 등장한 배경에는 소프트웨어 개발의 고질적인 문제가 있습니다. 비즈니스 로직이 프레임워크나 데이터베이스에 강하게 결합되면, 기술 스택을 변경하거나 테스트를 작성하는 것이 매우 어려워집니다. 예를 들어, JPA 어노테이션이 도메인 객체에 직접 붙어 있으면, 해당 도메인 로직을 테스트하려면 반드시 데이터베이스 연결이 필요하게 됩니다. 클린 아키텍처는 이런 문제를 해결하기 위해 **비즈니스 규칙을 기술적 세부사항으로부터 완전히 격리**하는 것을 목표로 합니다.
...
**핵심 비즈니스 규칙**이 있는 곳입니다. 회사 전체에서 공유되는 규칙입니다.
...
**애플리케이션 비즈니스 규칙**이 있는 곳입니다. "이 시스템에서" 어떻게 동작하는지 정의합니다.
...
**데이터 변환**을 담당합니다. Use Case가 원하는 형태 ↔ 외부 시스템이 원하는 형태.
...
**외부 도구들**이 있는 곳입니다. 언제든 교체할 수 있습니다.
