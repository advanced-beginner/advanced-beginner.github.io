...
{{< callout type="tip" title="비유: 건물 품질 검사" >}}
테스트 전략은 <strong>건물 품질 검사</strong>와 같습니다:

| 검사 단계 | 테스트 유형 | 특징 |
|----------|-----------|------|
| **벽돌 하나씩 검사** | 단위 테스트 | 빠르고 저렴, 가장 많이 수행 |
| **벽/기둥 조립 검사** | 통합 테스트 | 부품 간 연결 확인 |
| **건물 전체 준공 검사** | E2E 테스트 | 사용자 관점, 비용 높음 |

불량 벽돌을 사용하면 아무리 튼튼하게 조립해도 소용없습니다. <strong>도메인 모델(벽돌)을 철저히 테스트</strong>하는 것이 품질의 기본입니다.
{{< /callout >}}
