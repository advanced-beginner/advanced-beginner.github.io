...
Kotlin 컬렉션은 함수형 프로그래밍 스타일을 잘 지원합니다. `map`, `filter`, `reduce` 등의 고차 함수로 데이터를 선언적으로 변환할 수 있습니다.

{{< callout type="info" title="`{ it * 2 }`는 무엇인가요?" >}}
매개변수 1개짜리 람다(익명 함수)의 축약형입니다. `it`은 그 매개변수를 가리킵니다. 자세한 람다 문법은 [함수](../concepts/functions/)에서 다룹니다.
{{< /callout >}}
