...
Salting은 핫 키(데이터가 집중되는 키)를 분산시키는 기법으로, 3단계로 수행됩니다:
1. **키에 랜덤 접미사 추가**: `"hot_key"` → `"hot_key_0"`, `"hot_key_1"`, ... `"hot_key_9"`
2. **분산 처리**: 솔팅된 키로 집계하여 여러 파티션에서 병렬 처리
3. **접미사 제거 후 재집계**: 원래 키로 부분 집계 결과를 합산
...
> **파티셔닝 vs 버케팅**: 파티셔닝은 컬럼 값에 따라 **디렉토리를 분할**하는 것이고, 버케팅은 해시 함수를 사용하여 **파일 내부를 해시 분할**하는 것입니다. 파티셔닝은 필터링에 유리하고, 버케팅은 조인과 집계에 유리합니다.

| 구분 | 파티셔닝 | 버케팅 |
|------|---------|--------|
| 분할 방식 | 디렉토리 분할 (값 기준) | 파일 내 해시 분할 |
| 적합한 연산 | 필터링, 파티션 프루닝 | 조인, 집계 |
| 카디널리티 | 낮은 카디널리티 (연/월/지역 등) | 높은 카디널리티 (user_id 등) |
