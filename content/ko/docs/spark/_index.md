...
Apache Spark는 **대규모 데이터 처리를 위한 통합 분석 엔진**입니다. 하둡 MapReduce(기존 빅데이터 분산 처리 방식)보다 메모리에서 최대 100배, 디스크에서 10배 빠른 처리 속도를 제공하며, Java, Scala, Python, R 등 다양한 언어를 지원합니다.
...
RDD의 lineage(혈통) 정보를 통해 데이터 손실 시 자동으로 재계산합니다. 체크포인트 없이도 안정적인 처리가 가능합니다. RDD와 lineage에 대한 자세한 내용은 [RDD 기초](concepts/rdd/) 문서에서 다룹니다.
...
- 복잡한 ETL(데이터를 추출·변환·적재하는 파이프라인) 구축 시
...
*위 다이어그램은 백엔드 개발자, 데이터 엔지니어, 데이터 분석가 각 역할에 따른 권장 학습 순서를 보여줍니다.*
