...
클러스터에 접근 제어가 없다면, 주니어 개발자가 실수로 프로덕션 네임스페이스의 Deployment를 삭제하거나, CI/CD 파이프라인이 의도치 않게 모든 Secret을 읽을 수 있습니다. RBAC은 "누가 어디에서 무엇을 할 수 있는지"를 역할 기반으로 제어하여 이런 사고를 방지합니다.
