...
기본적으로 클러스터 내 모든 Pod는 서로 자유롭게 통신할 수 있습니다. 이는 편리하지만, 프론트엔드 Pod가 직접 데이터베이스에 접근하거나, 침해된 Pod 하나가 내부 전체를 탐색할 수 있다는 뜻이기도 합니다. NetworkPolicy는 이 기본 허용 상태에 방화벽 규칙을 추가하여 필요한 통신만 허용합니다.

NetworkPolicy는 Pod 수준의 방화벽 규칙을 정의하는 리소스입니다. NetworkPolicy를 설정하면 허용된 트래픽만 통과시킬 수 있습니다.
