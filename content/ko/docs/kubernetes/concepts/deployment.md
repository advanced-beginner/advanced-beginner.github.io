...
Pod를 직접 생성하면 어떻게 될까요? Pod가 죽으면 아무도 다시 살려주지 않고, 새 버전을 배포하려면 기존 Pod를 수동으로 삭제하고 다시 만들어야 합니다. 서비스 중단 없이 업데이트하는 것은 더욱 어렵습니다. Deployment는 이 문제를 해결합니다. 원하는 상태만 선언하면 Kubernetes가 자동으로 Pod를 생성, 업데이트, 복구까지 처리합니다.
