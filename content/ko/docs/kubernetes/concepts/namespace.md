...
하나의 클러스터에서 여러 팀이 작업하면 어떤 일이 벌어질까요? A 팀이 만든 `web-app` Deployment를 B 팀이 실수로 덮어쓰거나, 한 팀의 Pod가 클러스터 리소스를 독점해 다른 팀의 서비스가 영향받을 수 있습니다. Namespace는 이러한 충돌과 간섭을 방지하기 위해 하나의 물리 클러스터를 논리적으로 분리합니다.
