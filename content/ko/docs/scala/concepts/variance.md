...
lastmod: "2026-01-15"
title: 공변성 / 반공변성
description: "공변성과 반공변성의 작동 원리와 타입 안전성을 설명합니다."
weight: 13
---

## 전체 비유: 물류 창고 시스템

변성을 <strong>물류 창고 시스템</strong>에 비유하면 이해하기 쉽습니다:
...
`Dog`가 `Animal`의 서브타입일 때, `List[Dog]`를 `List[Animal]` 자리에 넘길 수 있을까요? 변성 규칙을 모르면 컴파일러가 왜 거부하는지 이해할 수 없고, `+A`나 `-A`를 언제 써야 하는지 감으로만 코딩하게 됩니다. 변성은 제네릭 타입의 안전한 대입 규칙입니다.
