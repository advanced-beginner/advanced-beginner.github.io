...
Option, Either, Future 등 여러 타입에서 `map`과 `flatMap`이 동일한 방식으로 동작한다는 것을 느꼈다면, 그것이 바로 Functor와 Monad입니다. 이 공통 패턴을 명시적으로 인식하면 새로운 타입을 만나도 즉시 사용법을 유추할 수 있고, 타입에 무관한 범용 유틸리티를 작성할 수 있습니다.
