...
Scala 컬렉션을 <strong>도서관</strong>에 비유하면 이해하기 쉽습니다:
...
- Scala는 <strong>불변 컬렉션</strong>을 기본으로 사용합니다 (스레드 안전, 예측 가능)
...
Java의 `ArrayList`에서 짝수만 골라 제곱값 리스트를 만들려면 루프, 조건문, 가변 리스트가 필요합니다. Scala 컬렉션은 `list.filter(_ % 2 == 0).map(x => x * x)`처럼 선언적으로 표현하여 의도를 즉시 드러내고, 불변성으로 동시성 버그를 원천 차단합니다.
