...
고급 타입 시스템을 <strong>건축 설계 규칙</strong>에 비유하면 이해하기 쉽습니다:
...
"이 함수는 `Int` 또는 `String`만 받습니다"를 `Any`로 표현하면 타입 안전성을 잃고, `Either`로 표현하면 래퍼가 번거롭습니다. Union Types, Opaque Types, Match Types 등은 이런 한계를 제거하여 도메인 규칙을 타입 레벨에서 정밀하게 표현할 수 있게 합니다.
