...
**Extension Method**
: 기존 타입에 새 메서드를 추가하는 기법. Scala 3에서는 `extension` 키워드 사용. [Type Class](#type-class) 구현에 활용. → [Scala 3 기능 비교](version-comparison/)
...
**Opaque Type (Scala 3)**
: 외부에서는 다른 타입으로 보이지만 내부에서는 기반 타입과 동일한 타입. 런타임 오버헤드 없는 타입 안전성. → [Scala 3 기능 비교](version-comparison/)
