...
애플리케이션 코드에 인덱스명 `products-v1`을 직접 하드코딩했는데, 매핑을 변경하여 `products-v2`로 전환해야 한다면? 모든 코드를 수정하고 배포해야 하며, 전환 시점에 다운타임이 발생합니다. Alias는 인덱스에 별명을 붙여 애플리케이션은 별명만 바라보게 하고, 실제 인덱스를 무중단으로 교체할 수 있게 합니다.
