...
"맥북"을 검색했을 때 수천 개의 문서가 일치한다면, 어떤 문서를 먼저 보여줘야 할까요? 단순히 입력 순서나 알파벳 순으로 정렬하면 사용자가 원하는 결과가 10페이지 뒤에 묻힐 수 있습니다. Score는 각 문서가 검색어와 "얼마나 관련 있는지"를 수치화하여, 가장 적합한 결과를 상위에 노출시킵니다.
...
BM25의 기본 점수만으로는 비즈니스 요구를 반영하기 어렵습니다. "프로모션 상품을 상위에 노출시키고 싶다", "상품명 일치가 설명문 일치보다 더 중요하다" 같은 요구사항은 기본 알고리즘이 알 수 없는 영역입니다. Boosting은 특정 필드나 조건에 가중치를 부여하여 비즈니스 로직을 검색 순위에 반영합니다.
...
단순 Boosting으로는 "최신 상품에 가산점을 주되 30일이 지나면 점수를 절반으로 줄이고, 판매량 100건 이상이면 추가 부스팅, 여기에 약간의 랜덤 요소도 넣고 싶다" 같은 복합적인 스코어링을 표현할 수 없습니다. Function Score Query는 여러 함수를 조합하여 이런 정교한 비즈니스 로직을 검색 순위에 반영합니다.
