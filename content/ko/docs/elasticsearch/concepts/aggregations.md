...
"카테고리별 상품 수"만으로는 부족합니다. "카테고리별 평균 가격은?", "카테고리 내 브랜드별 매출은?" 같은 다차원 분석이 필요한데, 단일 집계로는 이런 계층적 분석을 표현할 수 없습니다. 중첩 집계는 버킷 안에 추가 집계를 넣어 하나의 요청으로 다단계 분석을 수행합니다.
...
"월별 매출 합계"까지는 Bucket + Metric 집계로 구할 수 있습니다. 그러나 "이번 달 매출이 지난달보다 얼마나 증가했는가?" 또는 "월평균 매출은 얼마인가?" 같은 질문은 집계 결과를 다시 가공해야 답할 수 있습니다. Pipeline Aggregation은 다른 집계의 출력을 입력으로 받아 이런 2차 분석을 수행합니다.
