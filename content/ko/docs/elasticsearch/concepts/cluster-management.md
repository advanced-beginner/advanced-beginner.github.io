...
샤드가 특정 노드에 몰리면 어떻게 될까요? 해당 노드의 CPU가 100%에 도달하는 동안 다른 노드는 10%만 사용하여 검색 응답 시간이 10배 이상 증가합니다. 또한 Primary와 Replica가 같은 노드에 배치되면 해당 노드 장애 시 둘 다 유실됩니다. 샤드 할당 전략은 이런 불균형과 위험을 방지합니다.
