...
*Counter가 누적 증가하다가 프로세스 재시작 시 0으로 리셋되는 동작을 보여줍니다.*
...
*Gauge가 증가와 감소를 반복하며 현재 상태를 나타내는 동작을 보여줍니다.*
...
*Histogram 버킷이 누적 구조로 응답시간 분포를 저장하는 방식을 보여줍니다.*
...
*값이 증가만 하는지, 분포가 중요한지에 따라 Counter, Histogram, Gauge 중 적합한 타입을 선택하는 흐름을 보여줍니다.*
