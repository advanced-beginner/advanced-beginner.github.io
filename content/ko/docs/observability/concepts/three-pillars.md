...
*장애 분석 시 Metrics에서 이상을 감지하고, Logs로 원인을 파악하고, Traces로 경로를 추적하는 흐름입니다.*
...
*Counter, Gauge, Histogram, Summary 4가지 메트릭 타입과 각각의 대표적인 사용 예시를 보여줍니다.*
...
*하나의 Trace가 API Gateway, Order, Payment, Database 4개의 Span으로 구성되어 각 구간의 소요 시간을 보여줍니다.*
...
*Metrics, Logs, Traces 3요소가 상호 연결되어 에러율 감지 → 로그 필터링 → 트레이스 시각화로 순환하는 통합 분석 흐름입니다.*
