...
*Counter 원시값(누적)에 rate()를 적용하면 각 구간의 초당 변화율로 변환되는 과정을 보여줍니다.*
...
*5분간 6개 샘플을 수집하여 첫 값과 마지막 값의 차이를 시간으로 나누어 초당 증가율을 계산하는 과정입니다.*
...
*rate()는 전체 구간 평균을, irate()는 마지막 두 샘플만 사용하여 순간 변화율을 계산하는 차이를 보여줍니다.*
...
*Counter 리셋(프로세스 재시작) 발생 시 rate()가 자동으로 리셋을 감지하고 증가분만 계산하는 과정입니다.*
