...
histogram_quantile을 **진료 대기시간 분포 분석**에 비유하면 이해하기 쉽습니다:

| 대기시간 분석 비유 | histogram_quantile | 역할 |
|-----------------|-------------------|------|
| 평균 대기시간 | 평균 (avg) | 극단값에 왜곡됨 |
| 절반 환자 대기시간 | P50 (중앙값) | 일반적 경험 |
| 대부분 환자 대기시간 | P95 | 95% 환자 경험 |
| 최악 대기시간 | P99 | SLA 기준 |
| 대기시간 구간 (0-10분, 10-30분) | Histogram 버킷 | 분포 저장 구조 |
| 구간 간 추정 | 선형 보간 | 버킷 사이 값 추정 |
| 세밀한 구간 설정 | 버킷 설계 | 정확도 향상 |

이처럼 "평균 대기시간 10분"보다 "95% 환자가 30분 이내"가 더 정확한 정보인 것처럼, 백분위가 실제 경험을 반영합니다.

---

> **대상 독자**: 응답시간 분석이 필요한 개발자, SRE
> **선수 지식**: [메트릭 기초](../metrics-fundamentals/), [rate와 increase](rate-and-increase/)
> **소요 시간**: 약 25-30분
...
*대부분 사용자가 빠른 응답을 경험하지만 1%의 느린 요청이 평균을 왜곡하며, P99가 실제 최악 경험을 반영함을 보여줍니다.*
...
*Histogram 버킷이 누적 구조(le=0.1 ⊂ le=0.5 ⊂ le=1.0 ⊂ +Inf)로 데이터를 저장하는 방식입니다.*
...
*버킷 간격이 넓으면 선형 보간 오차가 크고, 세밀하면 실제 값에 가까운 정확한 백분위를 계산할 수 있음을 비교합니다.*
