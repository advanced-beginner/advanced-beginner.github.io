...
패턴 매칭은 Scala의 가장 강력한 기능 중 하나입니다. 값의 구조를 분석하고, 데이터를 추출하며, 조건에 따라 분기하는 것을 우아하게 처리합니다.
...
전체 값을 변수에 바인딩하면서 동시에 분해할 수 있습니다.
...
`unapply` 메서드를 정의하여 커스텀 패턴을 만들 수 있습니다.
...
`sealed` 타입은 컴파일러가 모든 케이스를 검사합니다.
